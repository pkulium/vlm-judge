...
for model_key in "${MODEL_KEYS[@]}"; do
    ...
    for task in "${TASKS[@]}"; do
        ...
        if [[ $task != *"cvrr"* ]]; then
            accelerate launch --num_processes=32 -m lmms_eval --model $model_key --model_args pretrained="pkulium/VideoLLava-demo" --tasks $task --batch_size 1 --limit 1000 --log_samples --log_samples_suffix "${model_key}_${task}" --task_mode score --output_path ./logs_p2_rebutal/
        else
            accelerate launch --num_processes=32 -m lmms_eval --model $model_key --model_args pretrained="pkulium/VideoLLava-demo" --tasks $task --batch_size 1 --log_samples --log_samples_suffix "${model_key}_${task}" --task_mode score --output_path ./logs/
        fi
    done
done
